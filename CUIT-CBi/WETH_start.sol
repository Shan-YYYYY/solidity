...
contract WETH9 {
    ...
    event DepositTo(address indexed addr, address indexed toAddress, uint256 wad);
    event WithdrawTo(address indexed addr, address indexed toAddress, uint256 wad);
    ...
    //选择提现地址
    function withdrawTo(address _toAddress, uint256 wad) public {
        require(balanceOf[msg.sender] >= wad);
        balanceOf[msg.sender] -= wad;
        payable(_toAddress).transfer(wad);
